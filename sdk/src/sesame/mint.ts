...
/**
 * The result of a successful sesame mint: the freshly-minted token id
 * (contract return value) and the hash of the confirmed transaction.
 */
...
/**
 * Mints a sesame warehouse-receipt token by invoking `mint()` on the
 * sesame-receipt contract.
 *
 * The custodian signs the transaction; the contract requires the caller to be a
 * registered custodian and rejects unregistered signers, non-sesame commodities
 * (only `SESAME` is valid), and zero bag-count / weight. Any such rejection —
 * as well as submission or confirmation failures — surfaces as a
 * {@link FarmledgeSDKError}.
 *
 ...
 * @param commodity       - Commodity code — must be `'SESAME'`
 * @param grade           - Grade label for the deposited lot (e.g. `'Grade A'`)
 ...
 * @returns The minted `tokenId` (format `SN-YYYY-NNNNNN`) and the confirmed `txHash`
